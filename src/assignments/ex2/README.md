...
1. **Invalid Formulas**: Returns `ERR_FORM` for malformed formulas.
2. **Circular Dependencies**: Detects cycles and flags them with `ERR_CYCLE`.

---

## How to Use

1. **Run the Program**: Launch the `Ex2GUI` class.
2. **Edit Cells**:  
   - Enter numbers, text, or formulas (e.g., `=A1*B2 + 3`).  
   - View computed results in real time.
3. **Save/Load**: Use the GUI or programmatic methods to save and reload spreadsheets.
...
This project was developed as part of Ariel University’s Intro to Computer Science course. It is based on guidelines provided by [Boaz Ben Moshe](https://docs.google.com/document/d/1-18T-dj00apE4k1qmpXGOaqttxLn-Kwi/edit).
